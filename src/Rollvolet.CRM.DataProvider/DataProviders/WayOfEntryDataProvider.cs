...
namespace Rollvolet.CRM.DataProviders
{
    public class WayOfEntryDataProvider : IWayOfEntryDataProvider
    {
        ...
        {
            var wayOfEntries = await _context.WayOfEntries.OrderBy(c => c.Position).ToListAsync();
        ...
        {
            var wayOfEntry = await _context.Interventions.Where(c => c.Id == id).Select(c => c.WayOfEntry).FirstOrDefaultAsync();

            if (wayOfEntry == null)
            {
                _logger.LogError($"No way-of-entry found for intervention with id {id}");
                throw new EntityNotFoundException();
            }
